//NEED TO TEST IT. INTEGRATION OF ALL SENSORS
//p1.24 for relay

/*
*for ultrasonic sensor detecting proximity:
*echo pin:P2.10(interrupt) ; trigger pin:  ;
...
int main()
{
  ...
  //LPC_PINCON->PINSEL1 &= 0xfffffff0; // Interface ECHO P0.16
	...
	LPC_PINCON->PINSEL4=1<<10; //P2.10 in function 1 for EINT0
	LPC_GPIO2->FIODIR=1<<11; //P2.11 in output mode (trigger)
	LPC_SC->EXTMODE=1; //P2.10 as edge triggered
	LPC_SC->EXTPOLAR=1; //P2.10 rising edge triggered
	NVIC_EnableIRQ(EINT0_IRQn); //enable the interrupt
...
{
		LPC_SC->EXTINT=1; //Clears the interrupt
	//while (!(LPC_GPIO0->FIOPIN & ECHO_PIN_W));// Wait till ECHO PIN becomes high
    timer_start();
    while (LPC_GPIO2->FIOPIN & ECHO_PIN_P); // Wait till ECHO PIN becomes low 
    echoTime = timer_stop(); // Store the time taken on stopping the timer 
